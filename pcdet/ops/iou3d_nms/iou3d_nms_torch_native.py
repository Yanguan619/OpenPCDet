...
def nms_gpu(boxes, scores, thresh, pre_maxsize=None, **kwargs):
    ...
    # 计算上三角 IoU 矩阵（利用对称性减少计算）
    if _HAS_NUMBA:
        # numba 快速路径：比 torch 逐对向量化实现快 ~15x
        iou_np = _nms_iou_matrix(np.ascontiguousarray(boxes.detach().cpu().numpy(), dtype=np.float32))
    else:
        iou_matrix = boxes_iou_bev(boxes, boxes)  # (N, N)
